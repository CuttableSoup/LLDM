...
| Area of effect | `targets.aoe` | **Authored but unused** — `entity_schema.toml` marks `targets = {number, aoe}` as UNUSED today; `fireball`'s own `targets = {number = 1, aoe = 5}` is inert data, nothing in `DM_Combat.py` reads it to hit more than one target. A real AoE conversion needs `_on_turn_detected`/`calculate_damage` extended to resolve against every hostile entity within `aoe` bands, not just `current_target` — out of scope for a data-only fix. |
