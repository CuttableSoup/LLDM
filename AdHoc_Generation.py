...
def generate_ad_hoc_creature(
    ...
    @brief Asks the local LLM whether phrase (the player's own message to ADaM) describes a
        ...
        creatures.toml's own wolf/bandit shape -- a wary/neutral/friendly conjured NPC is
        dialogue-only, same as a template author would choose for a peaceful NPC.
    ...
    if disposition == "hostile":
        ...
        if attack_skill:
            ...
            # Mirrors creatures.toml's own wolf/bandit shape exactly -- flee once genuinely
            # hurt (hp_per_remain under 0.40, the same cutoff rules.toml's "wounded" tier
            # bottoms out at), otherwise keep attacking until effectively dead.
            entity["behavior"] = [
                {
                    "requirements": [
